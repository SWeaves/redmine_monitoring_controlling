class McHumanResourceMgmtProjectController < ApplicationController
  ...
  def index    
    ...
    @statusesByAssigneds = Issue.find_by_sql("select assigned_to_id, subject, (select firstname from users where id = assigned_to_id) as assigned_first_name, (select lastname from users where id = assigned_to_id) as assigned_last_name,
